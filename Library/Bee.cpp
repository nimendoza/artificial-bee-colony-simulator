...
const float TOTAL_HOURS{ 24.0F / 3 };

const float Bee::CARRYING_CAPACITY{ 10.0F };
const float Bee::SPEED{ 100.0F * 3 };
const float Bee::BODY_RADIUS{ 10.0F };
const float Bee::TARGET_RADIUS{ 10.0F };
const float Bee::HARVESTING_DURATION{ 0.1F / 3 };
const float Bee::WORK_DURATION{ 16.0F / 3 };
const float Bee::REST_DURATION{ 8.0F / 3 };
const float Bee::EXTRACTION_YIELD{ 10.0F };
const float Bee::MAX_ENERGY{ 50.0F };
const float Bee::FATIGUE_PENALTY{ 24.0F * 3 };
const float Bee::ENERGY_CONSUMPTION_RATE{ 1.0F * 3 };
const std::map<const BeeType, float> Bee::LIFESPAN{
	{OnlookerBee, 24.0F * 42 / 3},
	{EmployeeBee, 24.0F * 42 / 3},
	{DroneBee, 24.0F * 30 * 3 / 3},
	{GuardBee, 24.0F * 42 / 3},
	{QueenBee, 24 * 30 * 24.0F / 3},
	{LarvaBee, INF},
	{EggBee, INF}
};
...
Bee::Bee(const Point& position, Hive& hive, const BeeType& type) : Entity(position, NORMAL_COLOR, ALERT_COLOR), hive{ hive }, foodsource(nullptr), body(BODY_RADIUS), face(Point(BODY_RADIUS, 2)) {
	...
	speed = std::normal_distribution<float>(SPEED, 10.0F * 3)(engine);
	harvestDuration = std::normal_distribution<float>(HARVESTING_DURATION, 0.05F / 3)(engine);
	workDuration = std::normal_distribution<float>(WORK_DURATION, 1.0F / 3)(engine);
	restDuration = TOTAL_HOURS - workDuration;
	maxEnergy = std::normal_distribution<float>(MAX_ENERGY, 20.0F)(engine);
	energy = maxEnergy;
	energyConsumptionRate = std::normal_distribution<float>(ENERGY_CONSUMPTION_RATE, 1.0F / 3.0F)(engine);
	fatiguePenalty = std::normal_distribution<float>(FATIGUE_PENALTY, 1.0F / 3)(engine);
	extractionYield = std::normal_distribution<float>(EXTRACTION_YIELD, 3.0F * 3)(engine);
	food = 0;
	lifespan = std::normal_distribution<float>(LIFESPAN.at(type), 24.0F / 3)(engine);
